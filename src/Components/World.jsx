...
const World = () => {
  const [article,setarticle] = useState([]);
      ...
      useEffect(() => {
          const fetchData = async () => {
            const data = await fetchCategoryData('world');
            setarticle(data);
            setLoading(false);
          };
  ...
  return (
    <div>
    <h2 className="text-4xl font-bold mb-6 text-center mt-8 font-Poppins">World News</h2>
  ...
  {article.description ? article.description.slice(0, 30) + "..." : "No description available."}
</p>
            <a
              href={article.url}
              target="_blank"
